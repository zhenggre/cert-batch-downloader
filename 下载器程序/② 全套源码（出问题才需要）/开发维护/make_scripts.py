# -*- coding: utf-8 -*-
"""生成 bat 脚本（用 GBK 写入，否则 cmd 窗口里的中文会变成乱码方块）。"""
...
(src1 := ROOT / "① 一键安装环境.bat").write_text(INSTALL, encoding="gbk", newline="\r\n")
(src2 := ROOT / "② 启动工具.bat").write_text(LAUNCH, encoding="gbk", newline="\r\n")
print("已生成：")
for f in (src1, src2):
    print("  %s  (%d 字节, GBK)" % (f.name, f.stat().st_size))
